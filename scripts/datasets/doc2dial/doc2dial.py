...
YOUR_LOCAL_DOWNLOAD = "/root/sharedtask-dialdoc2021/data"  # For subtask1, Doc2Dial v1.0.1 is already included in the folder "data".
...
class Doc2dial(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        ...
        datasets.BuilderConfig(
            name="doc2dial_testdev",
            version=VERSION,
            description="Load Doc2Dial dataset for shared task test-dev phase",
        ),
        datasets.BuilderConfig(
            name="doc2dial_test",
            version=VERSION,
            description="Load Doc2Dial dataset for shared task test phase",
        ),
    ]
    ...
    def _info(self):
        ...
        elif self.config.name in ["doc2dial_rc", "doc2dial_testdev", "doc2dial_test"]:
            features = datasets.Features(
                {
    ...
    def _split_generators(self, dl_manager):
        ...
        elif self.config.name == "doc2dial_testdev":
            return [
                datasets.SplitGenerator(
                    name=datasets.Split.VALIDATION,
                    gen_kwargs={
                        "filepath": os.path.join(
                            data_dir, "dialdoc21-sharedtask-phase1/doc2dial_dial_testdev_formatted.json"
                        ),
                    },
                ),
                datasets.SplitGenerator(
        ...
        elif self.config.name == "doc2dial_test":
            return [
                datasets.SplitGenerator(
                    name=datasets.Split.VALIDATION,
                    gen_kwargs={
                        "filepath": os.path.join(
                            data_dir, "dialdoc21-sharedtask-phase1/test_phase/doc2dial_dial_finaltest.json"
                        ),
                    },
                ),
                datasets.SplitGenerator(
                        ...
                        for dial in dials:
                            ...
                            for idx, turn in enumerate(dial["turns"]):
                                ...
                                if "answers" not in turn_to_predict:
                                    if "references" in turn_to_predict:
                                        ...
                                        turn_to_predict["answers"] = []
                                if turn_to_predict["answers"]:
                                    qa["answers"] = turn_to_predict["answers"]
                                yield id_, qa

        elif self.config.name in ["doc2dial_testdev", "doc2dial_test"]:
            """Load dialog data in the reading comprehension task setup, where context is the grounding document,
            input query is dialog history in reversed order, and output to predict is the next agent turn."""
                        ...
                        for dial in dials:
                            ...
                            for idx, turn in enumerate(dial["turns"]):
                                ...
                                if idx + 1 < len(dial["turns"]):
                                    ...
                                    # continue
                                    turn_to_predict = idx + 1
                                question_str = " ".join(
                                    list(reversed(all_prev_utterances))
                                ).strip()
                                question = " ".join(question_str.split()[:MAX_Q_LEN])
                                id_ = "{}_{}".format(dial["dial_id"], turn_to_predict) # For subtask1, the id should be this format.
                                qa = {
                                    "id": id_, # For subtask1, the id should be this format.
                                    "title": doc_id,
                                    "context": doc["doc_text"],
